...
它接管你的新标签页。打开的标签页按网站分组，哪个网站开了多少页，一眼就能看清。哪些开重了、哪些想留着回头再看，也不用再一个标签一个标签地翻。
...
| 主题色 | 八套，含跟随系统 |
| 呼出快捷键 | 默认 `⌘⇧K` / `Alt+Shift+K`，设置里能看到当前绑定 |
...
| 呼出快捷键 | `⌘⇧K` / `Alt+Shift+K` | 只能看当前绑定，改要去浏览器自己的快捷键页面（Firefox 上是 `Ctrl+Shift+L`） |
